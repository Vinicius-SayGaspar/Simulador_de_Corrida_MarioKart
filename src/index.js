...
async function logRollResult(characterName, block, diceResult, attribute) {
  console.log(
    `${characterName} 🎲 rolou um dado de ${block} e tirou ${diceResult} + ${attribute} = ${diceResult + attribute}`
  );
}
  ...
  for (let round = 1; round <= 5; round++) {
    ...
    if (block === "Confronto") {
      ...
      if (powerResult1 > powerResult2 && character2.Pontos > 0) {
        character2.Pontos--;
        console.log(
          `${character1.Name} ganhou!!! ${character2.Name} perdeu 1 ponto!🐢`
        );
      }
      ...
      if (powerResult2 > powerResult1 && character1.Pontos > 0) {
        character1.Pontos--;
        console.log(
          `${character2.Name} ganhou!!! ${character1.Name} perdeu 1 ponto! 🐢`
        );
      }

      console.log(
        powerResult2 === powerResult1 ? "Empate! Ninguém perde pontos" : ""
      );
    }
...
(async function main() {
  ...
  console.log(
    `🏎️ Jogadores: ${player1Selected.Name} e ${player2Selected.Name} \n`
  );
